...
export default defineComponent({
  setup() {
    const devSlug = useRoute()
    const state = reactive({
      isLoading: false,
      results: [] as any,
      resultsLen: false,
      currentPage: 1,
      devSlug: devSlug.params.slug,
      developers: String,
    ...
    watchEffect(() => {
      ...
      const fetchItems = async () => {
        const response = await axios(
          `https://api.rawg.io/api/games?key=c0233312e0954f0ca72e3d1e9d64ef32&developers=${state.devSlug}&page=${state.currentPage}&page_size=20`
        )
        state.results.push(response.data.results)
        state.nextPage = response.data.next
        const resp = await axios(
          `https://api.rawg.io/api/developers/${state.devSlug}?key=c0233312e0954f0ca72e3d1e9d64ef32`
        )
        state.developers = resp.data.name
