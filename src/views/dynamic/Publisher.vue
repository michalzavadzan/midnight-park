...
export default defineComponent({
  setup() {
    const pubSlug = useRoute()
    const state = reactive({
      isLoading: false,
      results: [] as any,
      resultsLen: false,
      currentPage: 1,
      pubSlug: pubSlug.params.slug,
      publishers: String,
    ...
    watchEffect(() => {
      ...
      const fetchItems = async () => {
        const response = await axios(
          `https://api.rawg.io/api/games?key=c0233312e0954f0ca72e3d1e9d64ef32&publishers=${state.pubSlug}&page=${state.currentPage}&page_size=20`
        )
        state.results.push(response.data.results)
        state.nextPage = response.data.next
        const resp = await axios(
          `https://api.rawg.io/api/publishers/${state.pubSlug}?key=c0233312e0954f0ca72e3d1e9d64ef32`
        )
        state.publishers = resp.data.name
